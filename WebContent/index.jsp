...
<script>
		var arr1, arr2, arr3, arr4, arr5, arr6; // Store throughput for each thread
		var speedWindow;
		var c5flag = 0;
		var c5trigger = 0;
		var c6flag = 0;
		...
		function calculatePreTest(){
			...
			function download(){
				...
				// ignore first 30*100ms = 3s, to eliminate TCP slow start impact 
				var c1 = 0, temp1 = 0, eload1 = 0, etemp1=0, xhrSpeed = 0,tempTime1 = 0;
				var c2 = 0, temp2 = 0, eload2 = 0, etemp2=0, xhr2Speed = 0,tempTime2 = 0;
				var c3 = 0, temp3 = 0, eload3 = 0, etemp3=0, xhr3Speed = 0,tempTime3 = 0;
				var c4 = 0, temp4 = 0, eload4 = 0, etemp4=0, xhr4Speed = 0,tempTime4 = 0;
				var c5 = 0, temp5 = 0, eload5 = 0, etemp5=0, xhr5Speed = 0,tempTime5 = 0;
				var c6 = 0, temp6 = 0, eload6 = 0, etemp6=0, xhr6Speed = 0,tempTime6 = 0;
				...
				xhr5.onprogress = $.throttle(100, function (e) {
					...
					xhr5Speed = ((e.loaded-etemp5)/1048576)/((t5-tempTime5)/1000);
					etemp5 = e.loaded;
					tempTime5 = t5;
				...
				xhr6.onprogress = $.throttle(100, function (e) {
					...
					xhr6Speed = ((e.loaded-etemp6)/1048576)/((t6-tempTime6)/1000);
					etemp6 = e.loaded;
					tempTime6 = t6;
				...
				xhr.onprogress = $.throttle(100, function (e) {
					...
					xhrSpeed = ((e.loaded-etemp1)/1048576)/((t1-tempTime1)/1000);
					etemp1 = e.loaded;
					tempTime1 = t1;
					...
					//if 6 threads run concurrently
					if(c6trigger == 1 && c1>10 && speedFinal == 0){
						// insert current speed into the speed sliding window of size 10
						var currentSpeed = xhrSpeed + xhr2Speed + xhr3Speed + xhr4Speed + xhr5Speed + xhr6Speed;
						//console.log(currentSpeed);
						speedWindow[speedCount] = currentSpeed;
						speedCount++;
						if(speedCount==10){
							speedCount = 0;
							windowFilled = 1;
						...
						// check average and difference
						if(windowFilled == 1){
							var sum = 0, avg = 0, highest=0, secondHighest=0, secondLowest=0, lowest = 9999;
							for(var i=0;i<speedWindow.length;i++){
								sum += speedWindow[i];
								if(speedWindow[i]>highest){
									secondHighest = highest;
									highest = speedWindow[i];
								}else if(speedWindow[i]<highest && speedWindow[i]>secondHighest){
									secondHighest = speedWindow[i];	
								}else if(speedWindow[i]<lowest){
									secondLowest = lowest;
									lowest = speedWindow[i];
								}else if(speedWindow[i]>lowest && speedWindow[i]<secondLowest){
									secondLowest = speedWindow[i];
								}
							}
							avg = sum/10;
							//console.log("avg = "+avg+", difference = "+(highest-lowest));
							if((secondHighest-lowest)/avg<0.1 || (highest-secondLowest)/avg<0.1){
								console.log("Time: "+(c1*0.1)+" s, Average = "+avg);
								speedFinal = 1;
							}
						}
					}else if(c5trigger == 1 && c1>10 && speedFinal == 0){
						var currentSpeed = xhrSpeed + xhr2Speed + xhr3Speed + xhr4Speed + xhr5Speed;
						//console.log(currentSpeed);
						speedWindow[speedCount] = currentSpeed;
						speedCount++;
						if(speedCount==10){
						...
						if(windowFilled == 1){
							var sum = 0, avg = 0, highest=0, lowest = 9999;
							for(var i=0;i<speedWindow.length;i++){
								sum += speedWindow[i];
								if(speedWindow[i]>highest){
									highest = speedWindow[i];
							...
							avg = sum/10;
							//console.log("avg = "+avg+", difference = "+(highest-lowest));
							if((highest-lowest)/avg<0.1){
								console.log("Time: "+(c1*0.1)+" s, Average = "+avg);
								speedFinal = 1;
							}
						}
					}else if(c1>10 && speedFinal == 0){
						var currentSpeed = xhrSpeed + xhr2Speed + xhr3Speed + xhr4Speed;
						//console.log(currentSpeed);
						speedWindow[speedCount] = currentSpeed;
						speedCount++;
						if(speedCount==10){
					...
					// Check if the previous 100ms reaches threshold speed
						if(c5trigger!=1||c6trigger!=1){
							var xhrTotalSpeed = xhrSpeed+xhr2Speed+xhr3Speed+xhr4Speed;
							if(c5trigger == 1){
								xhrTotalSpeed = xhrSpeed+xhr2Speed+xhr3Speed+xhr4Speed+xhr5Speed;
							}
							//console.log(xhrTotalSpeed);
							if(xhrTotalSpeed>3.125 && c5flag==0){
								temp5 = performance.now();
								xhr5.open("GET", picName+"5.jpg?" + new Date().getTime());
								c5flag = 1;//run this if statement only once
								c5trigger = 1;
									
								// gather information after 4 seconds slow start passed
								a1 = c1+40;
								a2 = c2+40;
								a3 = c3+40;
								a4 = c4+40;
							...
							if(xhrTotalSpeed>4.375 && c6flag==0){
				...
				xhr2.onprogress = $.throttle(100, function (e) {
					...
					xhr2Speed = ((e.loaded-etemp2)/1048576)/((t2-tempTime2)/1000);
					etemp2 = e.loaded;
					tempTime2 = t2;
					if(c2==a2){
						temp2 = t2;
				...
				xhr3.onprogress = $.throttle(100, function (e) {
					...
					xhr3Speed = ((e.loaded-etemp3)/1048576)/((t3-tempTime3)/1000);
					etemp3 = e.loaded;
					tempTime3 = t3;
					if(c3==40){
						temp3 = t3;
				...
				xhr4.onprogress = $.throttle(100, function (e) {
					...
					xhr4Speed = ((e.loaded-etemp4)/1048576)/((t4-tempTime4)/1000);
					etemp4 = e.loaded;
					tempTime4 = t4;
